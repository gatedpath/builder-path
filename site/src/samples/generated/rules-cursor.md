...
<!-- .cursor/rules/redbelly.mdc is generated by @gatedpath/agent-rules. Don't edit it here: edit packages/agent-rules/src/source.ts and run `npx -y @gatedpath/agent-rules --out .`. -->
...
Cursor applies this rule to every conversation (`alwaysApply: true`). It is a Project Rule under `.cursor/rules/`, the location Cursor documents for version-controlled rules.
