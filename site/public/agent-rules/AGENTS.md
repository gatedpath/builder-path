...
<!-- AGENTS.md is generated by @gatedpath/agent-rules. Don't edit it here: edit packages/agent-rules/src/source.ts and run `npx -y @gatedpath/agent-rules --out .`. -->
...
Codex CLI, GitHub Copilot, Cursor and other agents that follow the AGENTS.md convention read this file from the repository root. Nested AGENTS.md files in subdirectories take precedence for their subtree, so keep this one at the root.
