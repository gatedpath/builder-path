...
<!-- .github/copilot-instructions.md is generated by @gatedpath/agent-rules. Don't edit it here: edit packages/agent-rules/src/source.ts and run `npx -y @gatedpath/agent-rules --out .`. -->
...
GitHub Copilot reads this file for every chat and agent request in the repository. Path-specific instructions can be added as `.github/instructions/NAME.instructions.md` with an `applyTo` front matter.
